...
struct Beam {
    ...
    int operator - (const Beam& b) const {
        return (int)distance - (int)b.distance;
    }
};
...
public:
    ...
    void unpack(const velodyne_msgs::VelodynePacket &pkt, VPointCloud &pc, int ring);
    void unpackAndFilter(const velodyne_msgs::VelodynePacket &pkt,
                         VPointCloud &pc,
                         bot_core::pointcloud_t &pc_t,
                         int modulo, float distance, int pktnum);

    void setParameters(double min_range, double max_range, double min_angle,
                       double max_angle);
...
private:
    ...
    int velo_index[32]; // velodyne indices
    ...
    Beam old_beams[32];
    unsigned short int old_distances[32];

    void getPointFromBeam(const Beam& b,
                          VPoint& point);
